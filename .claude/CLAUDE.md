...
## Architecture
YAML-driven bot framework — swap verticals via `ACTIVE_VERTICAL` env var. 5 built-in verticals: `real_estate` (Alex), `home_services` (Sam), `legal` (Jordan), `dental` (Dana), `fitness` (Max). `/demo` endpoint works without GHL credentials. Conversation persistence via Redis.
